...
const UploadedPhotos = ({ photos, onChange }) => {
  ...
  const removePhoto = (ev, filename) => {
    ev.preventDefault();
    onChange([...photos.filter((listPhotoNew) => listPhotoNew !== filename)]);
  };

  const selectAsMainPhoto = (ev, filename) => {
    ev.preventDefault();
    onChange([filename, ...photos.filter((photoNew) => photoNew !== filename)]);
  };
